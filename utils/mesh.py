...
def generate_w_mapper(src_name_list, tgt_name_list, use_jax = False):
    ...
    for name in tgt_name_list:
        ...
        if name in ["browInnerUp", "cheekPuff"]:
            name_to_check += '_R'
        if name_to_check in list(src_name_to_idx.keys()):
...
def _jax_map_w(w, correspondences_list, brows_idxs = None, cheeks_idxs = None):
    ...
    if brows_in:
        w_in.at[brows_idxs[0]].set(max(w_in[brows_idxs[0]], w_in[brows_idxs[1]]))
    if cheeks_in:
        w_in.at[cheeks_idxs[0]].set(max(w_in[cheeks_idxs[0]], w_in[cheeks_idxs[1]]))
